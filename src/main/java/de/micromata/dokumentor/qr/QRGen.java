...
public class QRGen implements Creator {
  ...
  public byte[] create(HTTPGatewayContext hctx) {
    ...
    String contents =
        hctx.getQueryParameters()
            .get("contents")
            .orElseThrow(() -> new RuntimeException("Contents must be provided to the QR code"));
    ...
    switch (extension.toLowerCase()) {
      ...
      default:
        throw new RuntimeException(
            String.format(
                "Cannot use the specified format %s, must be one of png, jpg, gif, bmp",
                extension));
    }
  }
